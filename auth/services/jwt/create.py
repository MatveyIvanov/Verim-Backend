...
class CreateJWTTokens(ICreateJWTTokens):
    ...
    def __call__(self, user: UserType) -> JWTTokensSchema:
        return JWTTokensSchema(
            ...
            refresh=self._make_refresh(user)
        )
    ...
    def _make_access(self, user: UserType) -> str:
        ...
        payload = {
            'user': user.id,
            'exp': (current_time + timedelta(minutes=settings.ACCESS_TOKEN_LIFETIME)).timestamp(),
    ...
    def _make_refresh(self, user: UserType) -> str:
        ...
        payload = {
            'user': user.id,
            'exp': (current_time + timedelta(minutes=settings.REFRESH_TOKEN_LIFETIME)).timestamp(),
            'created_at': current_time.timestamp()
        }
        encoded = jwt.encode(
            payload=payload,
